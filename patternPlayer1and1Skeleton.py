...
#for every turn but the first we go here	
	...
	lastIndex = getLastIndexOf(oldThrows, *REPLACE*)

	if(lastIndex == *REPLACE*):
		output = random.choice(["R","P", "S"])

	else:
		throwToBeat = oldThrows[*REPLACE*]

		output = throwThatBeatsThis(*REPLACE*)
...
def getLastIndexOf(array, item):
	...
	#i ranges over all indices of the array
...
def throwThatBeatsThis(throwToBeat):
	*REPLACE*
	#theres a bunch ofl lines to write here
